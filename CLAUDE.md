...
```bash
python .claude/scripts/check_phase_status.py    # SessionStart hook — Phase 상태표
python .claude/scripts/test_for_file.py         # PostToolUse hook — 파일→테스트 매퍼
python .claude/scripts/run_all_unit_tests.py    # Stop hook — 비-블로킹 회귀
```
...
세 개의 Claude Code hook 이 검증을 자동화합니다. 정의는 `.claude/hooks.example.json` 에 보관되어 있고, `.claude/settings.local.json` 에 병합해야 활성화됩니다. 동일 스크립트는 수동 호출도 가능합니다.
...
`implement-phase` 스킬(`.claude/skills/implement-phase/SKILL.md`)이 Phase 구현을 주도합니다. 이 스킬의 절대 규칙:
