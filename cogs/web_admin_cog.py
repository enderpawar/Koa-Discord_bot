...
<html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Nothing Bot Admin</title><style>
:root{--bg:#f4f6f8;--panel:#fff;--line:#d8dee8;--line-strong:#c5ceda;--text:#16181d;--muted:#667085;--brand:#5865f2;--brand-soft:#eef0ff;--ok:#15803d;--ok-soft:#e8f7ee;--warn:#b7791f;--warn-soft:#fff7e6;--danger:#b42318}
...
.section{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:18px;margin-bottom:16px}.section h2{font-size:17px;margin:0 0 14px}
...
function render(){const g=state.selected;if(!g){$('status').textContent='봇이 접속한 서버가 없습니다.';return}guildId=String(g.id);const cfg=g.config||{};$('guilds').innerHTML='';for(const item of state.guilds){const b=document.createElement('button');b.className='guild'+(String(item.id)===guildId?' active':'');b.innerHTML=`${item.name}<br><span class="muted">${item.id}</span>`;b.onclick=()=>load(item.id);$('guilds').appendChild(b)}fillSelect($('tts_channel'),g.text_channels,cfg.tts_channel_id,'미설정');fillSelect($('leaderboard_channel'),g.text_channels,cfg.leaderboard_channel_id,'미설정');fillSelect($('voice_channel'),g.voice_channels,cfg.voice_channel_id,'미설정');$('post_time').value=cfg.leaderboard_post_time||'23:59';$('daily_enabled').checked=!!cfg.leaderboard_daily_enabled;const on=cfg.leaderboard_daily_enabled?'on':'';$('status').innerHTML=`<span class="status ${on}"><span class="dot"></span>일일 리더보드 ${cfg.leaderboard_daily_enabled?'켜짐':'꺼짐'}</span><p>TTS 입력: ${cfg.tts_channel_id?'#'+channelName(g.text_channels,cfg.tts_channel_id):'미설정'} · 음성 출력: ${cfg.voice_channel_id?channelName(g.voice_channels,cfg.voice_channel_id):'미설정'}</p>`}
