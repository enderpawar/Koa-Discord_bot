...
description: Implements ONE Phase of the Discord TTS Bot defined in docs/pipeline.md. Reads the Phase's referenced Skills (docs/skills/) and Rules (docs/rules/), satisfies the test contract in tests/unit/, and verifies via .Codex/scripts/check_phase_status.py. Use when the user says "implement phase N", "Phase N 구현", "phase N 진행", "다음 phase", or refers to building components mapped to a specific Phase. Always implements only ONE Phase per invocation.
---
...
2. 번호가 명시되지 않으면 다음 명령으로 현황 파악:
   ```
   python .Codex/scripts/check_phase_status.py
   ```
...
- 코멘트는 *why*만, *what*은 식별자가 설명 (AGENTS.md / project rules 준수)
...
이 스킬을 사용할 때 PostToolUse hook이 활성화되어 있다면(`.Codex/hooks.example.json` 참조), 파일을 Write/Edit 할 때마다 매핑된 단위 테스트가 자동 실행된다. 실패 시 stderr가 컨텍스트로 들어와 즉시 수정 가능.
