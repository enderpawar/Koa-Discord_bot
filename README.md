...
| 명령어 | 권한 | 설명 |
|---|---:|---|
| `/목소리` | 채널 관리 | TTS 목소리를 12종 중에서 고릅니다. `감정 표현` 목소리만 감정 톤을 냅니다. |
...
선택 가능한 목소리는 총 12종입니다. 이 중 `감정 표현` 이 붙은 2종만
문장 감정에 따라 톤이 바뀝니다 (아래 [감정 톤](#감정-톤) 참고).
...
- 문장의 감정에 맞춰 목소리 톤 바꾸기
- 너무 긴 메시지는 200자 근처에서 자르기
- 첨부만 있는 빈 메시지는 읽지 않기

### 감정 톤

메시지에 담긴 감정을 읽어 목소리 톤을 바꿉니다. 설정할 것은 없습니다.
...
`ㅋ` 하나처럼 애매한 신호나, `ㅋㅋㅠㅠ`처럼 웃음과 울음이 섞인 경우에는 톤을
바꾸지 않습니다. 잘못 얹은 감정이 밋밋한 낭독보다 더 거슬리기 때문입니다.

**중요: 한국어 목소리 대부분은 감정을 지원하지 않습니다.** Azure가 스타일을
제공하는 한국어 보이스가 매우 적습니다.

| 목소리 | 감정 |
|---|---|
| `여성 · 감정 표현` (Haena) | 13종 지원 |
| `남성 · 감정 표현` (Junho) | 12종 지원 |
| `남성 · 자연스러움` (InJoon) | `sad` 하나만 — `ㅠㅠ` 만 반영됨 |
| 나머지 9종 | **지원 안 함** (평소 톤으로 읽음) |

감정 톤을 쓰려면 `/목소리`에서 **`감정 표현`이 붙은 목소리**를 고르세요.

현재 목소리가 감정을 지원하는지는 `/상태`의 `감정 톤` 항목에서 바로 확인할 수
있습니다. 지원 목록은 봇이 시작할 때 Azure에 직접 물어보므로, 나중에 Azure가
지원 보이스를 늘리면 코드 수정 없이 따라갑니다. 지원하지 않는 목소리에서는
오류 없이 평소 톤으로 읽습니다.

`감정 표현` 목소리는 리전이나 구독에 따라 사용할 수 없을 수도 있습니다. 그 경우
`/상태`가 `이 목소리가 Azure 목록에 없습니다`로 알려줍니다.

### 발음 사전

관리자 대시보드에서 서버마다 **읽기 전에 바꿀 말**을 정할 수 있습니다.
`ㅇㅈ` → `인정`처럼 줄임말을 풀거나, 닉네임 발음을 교정할 때 씁니다.

- 읽을 말을 비우면 그 말은 소리 내지 않고 건너뜁니다.
- 대소문자는 구분하지 않습니다 (`gg` 규칙이 `GG`도 잡습니다).
- 규칙끼리 연쇄되지 않습니다. `가`→`나`, `나`→`다`를 넣어도 `가`는 `나`로만 읽습니다.
- 겹치면 긴 말이 이깁니다. `롤`과 `롤체`가 있으면 `롤체` 규칙이 먼저 적용됩니다.
- 최대 100개, 바꿀 말 20자·읽을 말 40자까지 저장할 수 있습니다.

현재 등록된 규칙 수는 `/상태`에서도 확인할 수 있습니다.
