...
- 버튼으로 파티 참가·대기·취소·마감 및 참가자 대화 스레드 생성, 시작 전 알림 제공 (끝난 모집은 자동 정리)
...
모집자와 현재 참가자는 모집글의 `스레드 만들기` 버튼으로 공개 대화 스레드를 한 번만
만들 수 있습니다. 이미 만들어진 뒤 버튼을 다시 누르면 같은 스레드로 안내합니다.
스레드는 24시간 동안 새 대화가 없으면 Discord가 자동 보관하며, 모집 메시지와 같은
채널을 볼 수 있는 서버 멤버에게는 보일 수 있습니다. 대기 중인 사용자는 참가자로
승격된 뒤 스레드를 만들 수 있습니다.
...
3. **OAuth2 -> URL Generator**에서 체크
   ...
   - Bot Permissions: `View Channels`, `Send Messages`, `Read Message History`, `Create Public Threads`, `Send Messages in Threads`, `Connect`, `Speak`, `Use Voice Activity`, `Use Slash Commands`
...
| 파티 스레드가 만들어지지 않음 | 봇에 `공개 스레드 만들기` 권한을 주고, 모집글이 일반 텍스트 채널에 있는지 확인 |
