...
톤은 Azure 뉴럴 보이스의 감정 스타일로 구현됩니다. **보이스마다 지원하는 스타일이
다르므로**, 봇은 시작할 때 Azure에 어떤 스타일을 쓸 수 있는지 물어보고 그 목록에
있는 것만 사용합니다. 선택한 목소리가 감정을 지원하지 않으면 평소 톤으로 읽고,
목록을 못 받아오면 감정 기능만 조용히 꺼집니다 — 읽기 자체는 영향받지 않습니다.
