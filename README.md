...
| `/파티모집` | 누구나 | 게임·정원·시작 시간을 정해 파티원을 모집합니다. 게임 이름은 자유 입력이고(`롤`, `발로`), 알림을 보낼 역할은 `태그` 옵션으로 따로 고릅니다(선택). |
...
`게임`은 그냥 이름을 적는 칸입니다. 서버에 그런 역할이 없어도 되고, `롤`·`발로`처럼
아무렇게나 적으면 됩니다. 알림을 보낼 역할은 `태그`에서 따로 고르며, 생략해도 됩니다.

```text
/파티모집 게임:롤 정원:5 시작:오늘 21:00 메모:일반 즐겜 태그:@LOL
```

`태그`로 고른 역할을 봇이 멘션할 수 없으면(역할의 멘션 허용이 꺼져 있고 봇에
`모든 역할 멘션` 권한도 없을 때) 파티는 그대로 열리고 알림만 생략됩니다.
...
`/파티모집`의 `태그`로 **멘션 허용이 꺼진 역할**까지 부르고 싶다면
...
- 물음표만 보낸 메시지(`?`, `??`, `????`)는 개수와 상관없이 `음?` 한 번으로 읽기
