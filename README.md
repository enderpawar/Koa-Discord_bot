...
`시작`은 `지금 바로` / `30분 뒤` / `1시간 뒤` / `2시간 뒤`, 그리고 아직 지나지
않은 `오늘 20:00`~`23:00`과 `내일 21:00` 중에서 고릅니다. 이미 지난 시간대는
목록에 나오지 않습니다.

`정원`을 `제한 없음`으로 두면 대기열이 생기지 않고 계속 받습니다. 인원을 정하면
정원이 찬 뒤 참가자는 대기열로 가고, 자리가 나면 자동으로 올라갑니다.
