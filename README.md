...
| `/파티모집` | 누구나 | 파티원을 모집합니다. 필수는 `제목` 하나이고, 비워 두면 **지금 바로 · 인원 제한 없음**으로 열립니다. 알림을 보낼 역할은 `태그` 옵션으로 따로 고릅니다(선택). |
...
필수 항목은 `제목` 하나입니다. 나머지를 비우면 **지금 바로 시작 · 인원 제한 없음**
으로 열립니다. 채팅에 "리썰 하실분" 한 줄 치는 것과 비슷한 속도로 쓰라고 만든
기본값입니다.

```text
/파티모집 제목:리썰 하실분              ← 지금 바로, 인원 제한 없음
/파티모집 제목:롤 칼바람 정원:5 시작:오늘 21:00 메모:일반 즐겜 태그:@LOL
```

`제목`은 그냥 적는 칸입니다. 서버에 그런 역할이 없어도 되고, `롤`·`리썰 3/4`처럼
아무렇게나 적으면 됩니다. Tab 을 누르면 **이 서버에서 최근에 쓴 제목**이 뜹니다.
알림을 보낼 역할은 `태그`에서 따로 고르며, 생략해도 됩니다.

`태그`로 고른 역할을 봇이 멘션할 수 없으면(역할의 멘션 허용이 꺼져 있고 봇에
`모든 역할 멘션` 권한도 없을 때) 파티는 그대로 열리고 알림만 생략됩니다.
이때 모집자에게만 그 사실을 알려줍니다.

`정원`을 비우면 인원 제한이 없어 대기열도 생기지 않습니다. 숫자를 적으면 정원이
찬 뒤 참가자는 대기열로 가고, 자리가 나면 자동으로 올라갑니다.

`시작`은 `지금`(기본값), `30분 뒤`, `오늘 21:00`, `내일 19:30`,
`2026-08-01 20:00` 형식을 지원합니다. 시간만 입력했고 오늘 이미 지난 시각이면
다음 날로 해석합니다.
